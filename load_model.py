...
from dags.scripts.preprocess import preprocess, features
import mlflow
...
def load_or_train_model():
    ...
    if os.path.exists(kmeans_model_path):
        ...
        # 데이터 로드 및 전처리        
        raw_data = pd.read_csv(os.path.join(os.getcwd(), 'dags\data', 'spotify_songs.csv'))
        data = raw_data.dropna(subset=['track_name'])

        model = KMeans(n_clusters=10, random_state=42)

        X, scaler = preprocess(data, model)
